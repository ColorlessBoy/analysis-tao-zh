 ...
 * wrap-math-v8.js
 *
 * Root fix: findMathCmd must CONTINUE searching when a found command is rejected
 * (letter follows). Previously it returned null, skipping other commands at the
 * same position. Now it advances to the next character and keeps searching.
 */
const fs = require('fs');
...
function findMathCmd(text, startAt) {
  // Scan from startAt, find first VALID math command
  for (let i = startAt; i < text.length; i++) {
    ...
    for (const c of SORTED_CMDS) {
      if (rest.startsWith(c)) {
        ...
        // ACCEPT if next char is NOT a letter
        if (!nextChar || !/[a-zA-Z]/.test(nextChar)) {
          return { cmd: c, start: i, end: nextPos };
        }
        // If next char IS a letter, this command doesn't match here.
        // Continue searching from i+1 for other commands.
      }
    }
    // No valid command at position i. Advance to i+1 and continue.
  }
  return null;
}
...
/**
 * Process math content (subscript/superscript) — NO $ wrappers.
 * Finds all commands and marks them (no wrapping needed, they're inside outer $...$).
 */
...
// TESTS
const tests = [
  ['a\\timesc = b\\timesc \\Rightarrow a = b does not work', 'timesc and arrow'],
  ['S = 1 + 1/2 + 1/4 + 1/8 + 1/16 + \\cdots .', 'series display'],
  ...
  ['xL = L.', 'short'],
  ['2S = 2 + 1 + 1/2 + 1/4 + 1/8 + \\cdots = 2 + S', 'series eq'],
  ['\\sqrt2', 'sqrt'],
  ['\\timesc', 'timesc'],
  ['\\times', 'times'],
...
tests.forEach(([input, desc]) => {
  ...
  console.log(desc + ':', JSON.stringify(out.substring(0, 120)));
});
...
let corrupt = 0;
lines.forEach((l, i) => {
  // Check for double $$ not at line start
  const trimmed = l.trim();
  if (l.match(/\$\$/) && !trimmed.startsWith('$$')) {
    console.log('DOUBLE $$ at line', i+1, ':', JSON.stringify(l.substring(0,80)));
    corrupt++;
  }
  // Check for $...$...$ pattern (unclosed)
  const m = l.match(/\$\$/g);
  if (m && m.length > 2 && !trimmed.startsWith('$$')) {
    console.log('POSSIBLE NESTED at line', i+1, ':', JSON.stringify(l.substring(0,80)));
  }
});
console.log('Total corrupted lines:', corrupt);
